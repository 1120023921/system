...
import cn.doublehh.system.vo.Result;
...
public class SystemController {
	...
	@ResponseBody
	public Result login(UsernamePasswordToken token,String checkCode, HttpSession session) {
		Result result = new Result();
		try {
			Subject subject = SecurityUtils.getSubject();
			subject.login(token);
			subject.getSession().setAttribute("USER", (String)subject.getPrincipal());
			result.setRole(userService.getUserByUid((String)subject.getPrincipal()).getRemark());
